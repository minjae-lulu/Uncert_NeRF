...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        # gt_image[i] = transforms.ToTensor()(gt_imgs[i])
...
def config_parser():
    ...
    parser.add_argument("--testskip", type=int, default=64, 
                        help='will load 1/N images from test/val sets, useful for large datasets like deepvoxels')
    ########################################################################################################   
    ...
    parser.add_argument("--i_weights", type=int, default=150, 
                        help='frequency of weight ckpt saving')
    parser.add_argument("--i_testset", type=int, default=150, 
                        help='frequency of testset saving')
    parser.add_argument("--i_video",   type=int, default=15000, 
                        help='frequency of render_poses video saving')

    ########################################################################################################    
    parser.add_argument("--total_epoch",   type=int, default=15000, 
                        help='total_epoch')
    ########################################################################################################    
    return parser
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        optimizer.zero_grad()
        img_loss = img2mse(rgb, target_s) # 1024 shape
        trans = extras['raw'][...,-1]
        loss = img_loss
